...
| OI-DEF-4 | Card-spend pre-settle pending balance | Two-state `Pending → Cleared` Ledger entries with `CardAuthorized` triggering the pending entry; only reopen if chargeback rates prove materially higher than v1 baseline assumes. |
