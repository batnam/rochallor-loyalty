...
Earn Source `FUND_TRANSFER`: the v1 catalogue routes `paymentType ∈ {FUND_TRANSFER, P2P_TRANSFER, QR_PAYMENT}` to this single Earn Source, and DSL rows discriminate via the canonical `paymentType` field preserved by the Bridge. Row 1 excludes `P2P_TRANSFER` with `FIXED 0` — defending against same-bank self-transfer farming, since rule `FIRST` semantics make the exclusion **stop** evaluation. Row 2 earns `1 pt / 2 USD` for USD-denominated transfers ≥ $1; row 3 earns `1 pt / 8,000 VND` for VND-denominated ones.
