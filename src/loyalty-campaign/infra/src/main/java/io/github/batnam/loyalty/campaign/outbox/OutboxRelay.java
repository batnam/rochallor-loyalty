package io.github.batnam.loyalty.campaign.outbox;
...
/**
 * Transactional-outbox relay (L3 §4 component 8). Two responsibilities:
 * <ul>
 *   <li>{@link #enqueue} — called <b>in the writing transaction</b> (Winner Selection) to stage an event.</li>
 *   <li>{@link #drain} — a ShedLock-guarded {@code @Scheduled} tick that publishes PENDING rows to MSK and
 *       marks them SENT in a <b>separate</b> transaction (retryable; at-least-once, dedup downstream).</li>
 * </ul>
...
public class OutboxRelay {
    ...
    @SchedulerLock(name = "campaign-outbox-relay")
    @Transactional
    public void drain() {
