...
/**
 * Single writer of {@code earning_audit_log}. Records every admin-originated Rule write as a
 * hash-chained, insert-only row: {@code rowHash = SHA-256(prevHash ‖ actor ‖ action ‖ entityType ‖
 * entityId ‖ before ‖ after)}. Any retroactive tampering breaks the chain (L3 §3.3).
 *
...
public class AuditLogWriter {
    ...
    public void record(String actor, String action, String entityType, String entityId,
        ...
        String prevHash = audit.findTopByOrderByAuditIdDesc()
                .map(EarningAuditLog::getRowHash)
                .orElse("");
        String rowHash = hash(prevHash, actor, action, entityType, entityId, beforeJson, afterJson);
        audit.save(EarningAuditLog.of(actor, action, entityType, entityId, beforeJson, afterJson, prevHash, rowHash));
    }
