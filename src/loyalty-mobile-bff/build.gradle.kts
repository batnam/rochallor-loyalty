// loyalty-mobile-bff — customer-facing edge API (C4 L2 BFFs; loyalty-mobile-bff.yaml).
// Aggregation only: no datastore, no Kafka. It fans out to loyalty-core / loyalty-redemption /
// loyalty-campaign over REST and composes their responses for the Mobile Banking App.
...
dependencies {
    ...
    // Stubs all three upstream services (core / redemption / campaign) in the IT — a BFF has no
    // datastore of its own, so the only integration surface is the outbound REST it aggregates.
