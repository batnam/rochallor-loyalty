 ...
 * HTTP status and its RFC-7807 {@code code} (e.g. {@code INSUFFICIENT_BALANCE}, {@code STEP_UP_REQUIRED})
 * so the edge contract surfaces the real cause instead of a blanket 500. A non-Problem body degrades to
 * a generic {@code UPSTREAM_ERROR} carrying the same status. Registered on every outbound {@link RestClient}
 * via {@code defaultStatusHandler(HttpStatusCode::isError, ...)}.
 */
@Component
