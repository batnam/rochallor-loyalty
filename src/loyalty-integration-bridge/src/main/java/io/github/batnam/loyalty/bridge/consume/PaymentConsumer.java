...
/**
 * Consume → validate → stamp → produce for {@code loyalty.ingress.payment.v1}.
 ...
 *. Schema-invalid messages go to the per-channel DLQ.
 */
@Component
public class PaymentConsumer {
    ...
    public void onPayment(ConsumerRecord<String, String> record) throws Exception {
        ...
        if (!mapping.isMapped(ingress.paymentType())) {
            // EMIT_FALLBACK_AND_ALERT: contract-valid but unrouted → inactive fallback source.
