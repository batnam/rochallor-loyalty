...
import io.github.batnam.loyalty.bridge.translate.CardSpendTranslator;
...
/**
 * The consume → validate → stamp → produce loop for {@code loyalty.ingress.card_spend.v1}
 * (ADR-0025). Schema-invalid or unparseable messages go to a per-channel DLQ and are NOT forwarded
 ...
 */
@Component
public class CardSpendConsumer {

    private static final Logger log = LoggerFactory.getLogger(CardSpendConsumer.class);
    ...
    public CardSpendConsumer(IngressSchemaValidator validator,
                             ObjectMapper mapper,
    ...
    @KafkaListener(topics = "${bridge.topics.ingress-card-spend}")
    public void onCardSpend(ConsumerRecord<String, String> record) throws Exception {
        JsonNode node;
        try {
        ...
        Set<ValidationMessage> errors = validator.validateCardSpend(node);
        if (!errors.isEmpty()) {
            toDlq(record, "schema validation failed: " + errors);
            return;
        }

        CardSpendIngress ingress = mapper.treeToValue(node, CardSpendIngress.class);
        ...
        ProducerRecord<String, String> out = new ProducerRecord<>(
                ...
                String.valueOf(earn.customerId()),   // partition key = customerId (ADR-0025)
                mapper.writeValueAsString(earn));
        propagateHeaders(record, out);
        kafka.send(out);
        log.debug("translated card_spend eventId={} customerId={}", earn.eventId(), earn.customerId());
    }
    ...
    private void toDlq(ConsumerRecord<String, String> record, String reason) {
        log.warn("routing card_spend to DLQ: {} (key={})", reason, record.key());
        ProducerRecord<String, String> dlq =
                new ProducerRecord<>(topics.cardSpendDlq(), record.key(), record.value());
