// loyalty-core — Membership + Ledger (C4 L3 loyalty-core; the hot critical path).
...
dependencies {
    // Onion rings (ADR-0001). This root project is :app; it depends on :infra (persistence +
    // messaging adapters) and :domain (the pure kernel). The dependency rule runs app → infra → domain.
    ...
    // Distributed lock for multi-pod @Scheduled jobs (Expiry, TTL Sweeper) — L3 §3.3.
    ...
    // JSR-310 for Instant so outbox / Kafka payloads serialize the same way the bridge emits them.
