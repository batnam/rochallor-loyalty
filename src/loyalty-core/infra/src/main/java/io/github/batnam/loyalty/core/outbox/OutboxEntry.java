...
/**
 * Transactional-outbox staging row (L3 §3.2). Inserted in the same transaction as the business
 * write; drained to MSK by the Outbox Relay in a separate, retryable transaction — at-least-once
 * delivery with same-transaction durability, without a distributed Postgres↔Kafka transaction.
 */
@Entity
@Table(name = "outbox")
