...
/**
 * Single writer of {@code redemption_audit_log}. Records every admin-originated Reward write as a
 * hash-chained, insert-only row: {@code rowHash = SHA-256(prevHash ‖ actor ‖ action ‖ entityType ‖
 * entityId ‖ before ‖ after)}. Any retroactive tampering breaks the chain (L3 §5).
 *
 * <p>Runs {@code Propagation.MANDATORY} — it must enlist in the caller's transaction so the audit row
...
public class AuditLogWriter {
    ...
    public void record(String actor, String action, String entityType, String entityId,
        ...
        String prevHash = audit.findTopByOrderByAuditIdDesc()
                .map(RedemptionAuditLog::getRowHash)
                .orElse("");
        String rowHash = hash(prevHash, actor, action, entityType, entityId, beforeJson, afterJson);
        audit.save(RedemptionAuditLog.of(actor, action, entityType, entityId, beforeJson, afterJson, prevHash, rowHash));
    }
